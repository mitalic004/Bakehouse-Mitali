...
export class BakehouseStack extends Stack {
	constructor(scope, id, props) {
		...
		// Tags
		// ----------------------------------
		// Adds owner & project tags to all resources created in stack
		...
		// ----------------------------------
		// Adding created IAM managed policy
		...
		// ----------------------------------
		const githubOrg = props.githubOrg || "mitalic004"
		const githubRepo = props.githubRepo || "Bakehouse-Mitali"
		...
		const githubActionsDeployRole = new iam.Role(
			...
			{
				roleName: "github-actions-mitali-chavan-bakehouse",
		...
		// Create a parameter group that forces SSL to RDS
		const postgresParameterGroup = new rds.ParameterGroup(
			...
			"postgres-parameter-group",
			{
				...
				removalPolicy: cdk.RemovalPolicy.DESTROY, // default retain
				parameters: {
		...
		const cluster = new rds.DatabaseCluster(this, "rds-cluster", {
			...
			// Private subnets, all internal traffic, not over internet
			vpc: sharedVpc,
			vpcSubnets: {
			...
			serverlessV2MinCapacity: 0.5, // min units at any given time, always active
			serverlessV2MaxCapacity: 1, // max units before throttle

			// Needed for the Data API from our Lambdas
			// Allow things to connect to, send & receive data from cluster
			enableDataApi: true,
		...
		const usersTable = new dynamodb.Table(this, "users-table", {
			...
			// Defining which column to user as key, (usually main aspect needed for data retrieval - finding user account by email)
			partitionKey: {
				name: "email",
				type: dynamodb.AttributeType.STRING,
			},

			// Billed every time a request is made / DB is accessed
		...
		const productCardsBucket = new s3.Bucket(this, "product-cards", {
			...
			removalPolicy: cdk.RemovalPolicy.DESTROY,
			autoDeleteObjects: true, // empties objects first when bucket is destroyed
			publicReadAccess: true,
			// Has to have secure data transfer method
			encryption: s3.BucketEncryption.S3_MANAGED,
			blockPublicAccess: new s3.BlockPublicAccess({
				blockPublicAcls: true, // not allowing general users to put bucket / objects
		...
		const clientBucket = new s3.Bucket(this, "client-bucket", {
			...
			publicReadAccess: false, // will add resources policies to enable access to bucket when needed
			encryption: s3.BucketEncryption.S3_MANAGED,
		...
		// Deny all actions on bucket & objects inside if connection is not secure
		...
		// Shared client query policy, send query params through CloudFront without caching them
		// CloudFront caches by default
		// Will be attached to all cloudfront distributions
		...
		// Lambda bundling
		// ----------------------------------
		// Not used anymore, bundling lambdas ahead of time
		const bundling = {
			externalModules: ["aws-sdk"],
			nodeModules: ["data-api-client"],
		}

		// Env vars for all lambdas
		const lambdaEnvVars = {
			NODE_ENV: "production",
			AWS_NODEJS_CONNECTION_REUSE_ENABLED: "1", // allows reconnection to DB without opening new session
			DB_NAME: props.dbName,
			CLUSTER_ARN: cluster.clusterArn, // know where cluster info is stored
			SECRET_ARN: cluster.secret?.secretArn || "NOT_SET", // allows connection to DB cluster, stored in secrets manager
		...
		const api = new apigw.RestApi(this, "apigw", {
			...
			deploy: true, // redeploying every time CDK stack is deployed, in case there are updates, false for testing
			deployOptions: {
				stageName: "api", // tells CloudFront to route to specific place, request is for API gateway instead of S3 bucket
			},
